 ...
 **ids** | **int**| Berufs-ID einer Berufsbezeichnung (z.B. 6133 für Industriemeister/in - Lebensmittel). Mehrere Komma-getrennte Angaben möglich. | [optional]
...
with weiterbildungssuche.ApiClient(configuration) as api_client:
    ...
    try:
        # Weiterbildungssuche
        api_instance.weiterbildungssuche(page=page, sys=sys, sw=sw, ssw=ssw, ids=ids, orte=orte, uk=uk, re=re, bt=bt, uz=uz, dauer=dauer, uf=uf, ban=ban, it=it, bg=bg, ortsunabhaengig=ortsunabhaengig, sort=sort)
    except weiterbildungssuche.ApiException as e:
